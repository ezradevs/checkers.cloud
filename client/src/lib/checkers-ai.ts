import { evaluatePosition, getLegalMovesWithComplex, analyzePosition, getSquareCoordinates } from './checkers-logic';
...
export function minimax(
  ...
  const moves = getLegalMovesWithComplex(position, player, colorComplex, rules);
  ...
  if (maximizingPlayer) {
    ...
    for (const move of moves) {
      ...
      const evaluation = minimax(newPosition, depth - 1, false, player === 'red' ? 'black' : 'red', colorComplex, alpha, beta, rules);
    ...
    for (const move of moves) {
      const newPosition = makeMove(position, move);
      const evaluation = minimax(newPosition, depth - 1, true, player === 'red' ? 'black' : 'red', colorComplex, alpha, beta, rules);
...
function evaluatePositionAdvanced(position: BoardPosition): number {
  ...
  // Mobility evaluation
  const redMobility = getLegalMovesWithComplex(position, 'red', false).length;
  const blackMobility = getLegalMovesWithComplex(position, 'black', false).length;
...
export function findBestMoveWithDepth(
  ...
  const legalMoves = getLegalMovesWithComplex(position, player, colorComplex, rules);
  ...
  sortedMoves.forEach(move => {
    const newPosition = makeMove(position, move);
    const score = minimax(newPosition, depth - 1, player === 'black', player === 'red' ? 'black' : 'red', colorComplex, -Infinity, Infinity, rules);
...
function getAdvancedMoveExplanation(
  ...
  const opponentMoves = getLegalMovesWithComplex(position, currentEval > 0 ? 'black' : 'red', false);
